...
-- The major difference between Primary key and foreign key is Primary keys uniquely identify a record in a table, while foreign keys establish a link between tables. primary key is not null and a unique value whereas foreign key is a primary key of an another table which is used as a reference in the 1st table.
